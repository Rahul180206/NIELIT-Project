...
class ProctoringEngineV11:
    ...
    def load_gaze_config(self, path):
        ...
        print("\n=== GAZE CALIBRATION ===")
        print("LEFT  ->", self.left_thresh)
        print("RIGHT ->", self.right_thresh)
        print("UP    ->", self.up_thresh)
        print("=========================\n")
        ...
        with self.mpfm.FaceMesh(**self.fm_args) as fm:
            ...
            while True:
                ...
                if res.multi_face_landmarks:
                    ...
                    # ---------------- EAR ----------------
                    ear = (
                        compute_EAR(pts, LEFT_EYE_IDX)
                        + compute_EAR(pts, RIGHT_EYE_IDX)
                    ) / 2.0
                    ...
                    if self.closed_frame_count >= LONG_CLOSE_FRAMES:
                        self.logger.log("LONG_EYE_CLOSURE", self.frame_no, ear=float(ear))
                    ...
                    talking_frame = (
                        mar > TALK_MAR_THRESH
                        or (mar > 0.12 and mar_delta > RAPID_MAR_DELTA)
                    )
                    ...
                    if self.talk_count >= TALK_SUSTAINED_FRAMES:
                        self.logger.log("TALKING", self.frame_no, mar=float(mar))
                    ...
                    if liris is None:
                        liris = ((lbox[0]+lbox[2])/2, (lbox[1]+lbox[3])/2)
                    if riris is None:
                        riris = ((rbox[0]+rbox[2])/2, (rbox[1]+rbox[3])/2)
                    ...
                    gaze_label = self.classify_gaze(nx, ny)
